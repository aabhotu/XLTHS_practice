...
N=70;
b=fir1(N,[wn1 wn2], 'stop'); w=0:2*pi/551:pi; h1=freqz(b,1,w);
subplot(311); plot(w/pi,(20.*log10(abs(h1))));
title('Dap ung tan so cua bo loc H1'); grid on;
y=filter(b,1,x);
y_fft=fft(y); subplot(312); plot(abs(y_fft));
title('tin hieu sau khi loc y trong mien tan so'); grid on;

figure (3);
fc1=350; fc2=1000;
w_han=hanning(70); 
wn1=2*fc1*pi/Fs; wn2=2*fc2*pi/Fs;
hd=thongthap(pi,70)- thongthap(wn2,70) + thongthap(wn1,70);
h=hd.*w_han';
...
y=filter(h,1,x); 
y_fft=fft(y);subplot(312);plot(abs(y_fft));
title('tin hieu sau loc y trong mien tan so'); grid on;
